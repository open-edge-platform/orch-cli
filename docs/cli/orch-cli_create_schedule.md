...
```
# Create a new repeated schedule, an osupdate, using days of week (target by resource ID)
...
```
  ...
  -T, --target string             Target maintenance on a host|region|site: resource ID (host-abcd1234|region-abcd1234|site-abcd1234) or name with type prefix (host:name|region:name|site:name)
