...
// static/ is populated by running: cd kvm-angular-app && ng build
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Binary helpers (AMT protocol encoding)
// ─────────────────────────────────────────────────────────────────────────────
...
func hexMD5amt(s string) string {
	//nolint:gosec // MD5 required by AMT Digest auth RFC 2617 — not used for security
	h := md5.Sum([]byte(s))
...
// decodeUTF8Binary - MPS sends binary data in TEXT WebSocket frames as UTF-8.
...
func connectToMPSRelay(relayURL, jwtToken string, logf func(string, ...interface{})) (*kvmSession, error) {
	...
	if token == "" {
		return nil, fmt.Errorf("relay URL missing 'token' query parameter")
	}
	guid := parsed.Query().Get("host")

	// Rebuild the relay URL with the params the MPS relay handler requires.
	// The inventory URL only carries token+host; the relay also needs p, port,
	// tls and mode to know how to connect to the AMT device.
	// Token is passed as Sec-WebSocket-Protocol, not as a query param.
	...
	dialer := websocket.Dialer{
		TLSClientConfig: &tls.Config{InsecureSkipVerify: true}, //nolint:gosec // cluster uses self-signed cert
	}
	headers := http.Header{}
...
// ─────────────────────────────────────────────────────────────────────────────
// openBrowser — open the default browser at the given URL.
// ─────────────────────────────────────────────────────────────────────────────
...
func openBrowser(url string) {
	...
	default:
		// Unsupported OS or no display — URL already printed above, nothing more to do.
		return
	}
	if err := exec.Command(cmd, args...).Start(); err != nil { //nolint:gosec // url is constructed locally
		// Best-effort only — URL was already printed to stdout.
	}
}
...
func startKVMViewer(ctx context.Context, relayURL string, hostClient infra.ClientWithResponsesInterface, projectName, hostID string) error {
	...
	sess, err := connectToMPSRelay(relayURL, jwtToken, logf)
	if err != nil {
	...
	// Step 3 — print URL and attempt to open browser.
	// Wait 1s so VS Code Remote SSH port-forwarding can detect the new listener
	// and make the URL directly clickable in the terminal.
	time.Sleep(1 * time.Second)
	fmt.Printf("\n╔══════════════════════════════════════════════════════╗\n")
	fmt.Printf("║  KVM session is LIVE — open this URL in your browser ║\n")
	fmt.Printf("╚══════════════════════════════════════════════════════╝\n")
	fmt.Printf("\n  %s\n\n", viewerURL)
	fmt.Printf("  (In VS Code: Ctrl+click the URL above — port is auto-forwarded)\n")
	fmt.Printf("  (Over plain SSH: ssh -L %d:localhost:%d <user>@<host>)\n\n", port, port)
	...
	// Step 7 — verify KVM_STATE_STOP (best-effort, timeout 30s)
