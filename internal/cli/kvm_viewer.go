...
//
// When serving index.html it injects a one-time session token as:
//
//	<script id="__orch-token">window.__ORCH_TOKEN = "<hex>";</script>
//
// Angular reads window.__ORCH_TOKEN on init, stores it in process memory,
// and attaches it as X-Session-Token on every API/WebSocket request.
type spaHandler struct {
	fileServer   http.Handler // serves static files from the embedded FS
	efs          fs.FS        // embedded Angular build output
	sessionToken string       // 256-bit crypto token injected into index.html; never exposed in URL or logs
}

func newSPAHandler(efs fs.FS, sessionToken string) spaHandler {
	return spaHandler{
...
func (h spaHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// Inject token into index.html before serving.
	if path == "index.html" {
		h.serveIndexWithToken(w, r)
		return
	}
	...
	// Unknown path — let Angular router handle it via index.html.
	h.serveIndexWithToken(w, r)
}

// serveIndexWithToken serves index.html with a one-time session token injected
// as <script id="__orch-token">window.__ORCH_TOKEN="TOKEN";</script> before </head>.
// Cache-Control: no-store ensures each load gets a fresh token.
// Angular reads the token once, deletes window.__ORCH_TOKEN, and removes the
// <script> element — leaving no trace of the token in the DOM.
func (h spaHandler) serveIndexWithToken(w http.ResponseWriter, _ *http.Request) {
	data, err := fs.ReadFile(h.efs, "index.html")
	if err != nil {
		http.Error(w, "index.html not found", http.StatusInternalServerError)
		return
	}
	// id="__orch-token" lets Angular locate and remove this node after reading.
	injection := `<script id="__orch-token">window.__ORCH_TOKEN="` + h.sessionToken + `";</script>`
	modified := strings.Replace(string(data), "</head>", injection+"</head>", 1)
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	// Prevent caching of the injected page so the token is always fresh.
...
type kvmServer struct {
	...
	mu           sync.RWMutex
	sessionToken string // crypto/rand 256-bit token injected into index.html; validated on every /api/* and /ws/kvm request
}

// requireToken is HTTP middleware that validates X-Session-Token on every
// /api/* and /ws/kvm request. The token is generated by orch-cli at startup,
// injected once into index.html, and stored in Angular process memory.
// Any request without the correct token is rejected with 403.
func (srv *kvmServer) requireToken(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		got := r.Header.Get("X-Session-Token")
		if got == "" {
			// WebSocket upgrades cannot set custom headers from the browser;
			// accept the token via query param for the /ws/kvm endpoint only.
			got = r.URL.Query().Get("token")
		}
		if got != srv.sessionToken {
			fmt.Printf("[KVM] token REJECTED %s %s (got %d chars)\n", r.Method, r.URL.Path, len(got))
			http.Error(w, "forbidden", http.StatusForbidden)
			return
		}
		fmt.Printf("[KVM] token OK %s %s\n", r.Method, r.URL.Path)
		next(w, r)
	}
}

// serveStatus returns the current KVM session state as JSON {"state":"..."}.
...
// serveDisconnect handles POST /api/disconnect — closes the AMT session cleanly.
...
func mpsRelayTLSConfig(caPath string, logf func(string, ...interface{})) (*tls.Config, error) {
	...
	// TLS 1.3 minimum — matches SoL session and satisfies SDL430-12/SDL523-13.
...
func startKVMViewer(ctx context.Context, token, mpsDomain, deviceGUID, orchCA string, hostClient infra.ClientWithResponsesInterface, projectName, hostID string) error {
	...
	// Generate a 32-byte (64-char hex) cryptographically random session token.
	// Injected into index.html as window.__ORCH_TOKEN — never appears in the URL.
	sessionToken := randomHex(32)
	fmt.Printf("[KVM] session token generated (first 8 chars: %s...)\n", sessionToken[:8])
	...
	srv := &kvmServer{
		...
		sessionToken: sessionToken,
		upgrader: websocket.Upgrader{
			// SDL430-12/SDL523-13: restrict WebSocket upgrades to localhost origins.
			// The server only listens on 127.0.0.1; reject any request whose
			// Origin header points outside localhost.
			CheckOrigin: func(r *http.Request) bool {
				origin := r.Header.Get("Origin")
				if origin == "" {
					return true // non-browser client (e.g. fuzz test)
				}
	...
	mux.HandleFunc("/ws/kvm", srv.requireToken(srv.serveKVMWebSocket))
	...
	mux.Handle("/", newSPAHandler(embeddedFS, sessionToken))
